...
from tools.graph.resolver.neighborhood_format import empty_function_payload, node_record, path_excerpt
from tools.graph.resolver.verified_taint_paths import get_verified_taint_paths
    ...
    def serialize_function_neighborhood(
        ...
        try:
            ...
            if not records:
                payload = {
                    "function": {
                        **empty_function_payload(func_name)
                    },
                    "concurrency": {
                        "hosting_task": None,
                        "task_priority": None,
            ...
            for caller in payload.get("upstream", {}).get("standard_callers", []) or []:
                ...
                if caller_name:
                    nodes.append(node_record(caller_name, caller.get("type", "Function"), caller.get("id"), source_kind="caller"))
                    edges.append({
            ...
            for net in payload.get("upstream", {}).get("network_triggers", []) or []:
                ...
                if net_name:
                    nodes.append(node_record(net_name, net.get("type", "NetworkSignal"), net.get("id"), source_kind="signal"))
                    edges.append({
            ...
            for source_name in source_names[:5]:
                for sink_name in sink_names[:5]:
                    path_candidates.append({
                        "type": "source_to_sink",
                        "path": path_excerpt(func_name, source_name, sink_name),
                        "source": source_name,
                        "sink": sink_name,
                        "status": "inferred",
                        "resolution": "neighborhood_combination",
                        "provenance": "function_neighborhood_serializer",
            ...
            if not path_candidates:
                for sink_name in sink_names[:5]:
                    path_candidates.append({
                        "type": "internal_sink",
                        "path": path_excerpt(func_name, None, sink_name),
                        "source": None,
                        "sink": sink_name,
                        "status": "inferred",
                        "resolution": "neighborhood_combination",
